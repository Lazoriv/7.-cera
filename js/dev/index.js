import { b as bodyLockStatus, g as gotoBlock, u as uniqArray } from "./common.min.js";
(function polyfill() {
...
const eyes = document.querySelectorAll('[class^="craft-eye"]');
...
function parallax() {
  ...
  const scrollProgress = rect.top * -1;
  items.forEach((item, index) => {
    const speed = (index + 1) * 0.04;
    const yMove = scrollProgress * speed;
    item.style.transform = `translateY(${yMove}px)`;
  });
  requestAnimationFrame(parallax);
}
parallax();
...
function initSliders() {
  if (document.querySelector(".workshop__slider")) {
    new Swiper(".workshop__slider", {
